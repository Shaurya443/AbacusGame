...
//used the create the elements of the bead
function UIElement(x, y, width, height, type, ref, subref, slotType) {
  ...
  //used to create the beads and lines
  function Bead() {
  ...
  // abacus type
  function AbacusCtrl(slotType) {
    
    // used to create the bead lines
    this.beadLines = 8

//used to create the beads on the line
    this.beadPerLine = 5

    // used to create the top line where the 5th bead is
    this.beadSep = 3

    // bead height,spacing,width
    this.beadHeight = 40;
    this.beadSpacing = 80;
    this.beadWidth = 60;
...
// I DONT KNOW
    ...
    this.activated = function(nodeId) {
      ...
      // is used the move the beads up and down the line
  ...
  function Abacus(parentDivId, type) {
    ...
    function drawBead(nodeId, ctx) {
        ...
        var dn = new UIElement(nodePosX+2, nodePosY+2, abacusCtrl.beadWidth, abacusCtrl.beadHeight-4, 0, nodeId, 0, 0);
  
        ctx.fillStyle = "rgba(100, 60, 60, 0.3)";
        drawRoundRectFilled(ctx, dn.x+3, dn.y+3, dn.x2-dn.x, dn.y2-dn.y, 15);
        ...
        drawRoundRectFilled(ctx, dn.x, dn.y, dn.x2-dn.x, dn.y2-dn.y, 19);
